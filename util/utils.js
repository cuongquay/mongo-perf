...
function generateRandomString(length) {
	buf = "";
	for ( i = 0; i < length; i++) {
		buf += possible.charAt(Math.floor(Math.random() * possible.length));
		;
	}
	return buf;
}
...
function formatRunDate(now) {
	...
	return (1900 + now.getYear() + "-" + pad(now.getMonth() + 1) + "-" + pad(now.getDate()));
}

function runTest(test, thread, multidb, multicoll, runSeconds, shard, crudOptions, printArgs, cleanUp) {
	...
	for (var i = 0; i < multidb; i++) {
		...
		if (shard == 1) {
			for (var j = 0; j < multicoll; j++) {
				...
				collections[(multicoll * i) + j].ensureIndex({
					_id : "hashed"
				});
			}
	...
	if ("totalOps/s" in result) {
		...
		total = result["insert"] + result["query"] + result["update"] + result["delete"] + result["getmore"] + result["command"];
	}
	error_string = "";
 ...
 */
function runTests(threadCounts, multidb, multicoll, seconds, trials, includeFilter, excludeFilter, shard, crudOptions, excludeTestbed, printArgs, cleanUp) {
	...
	if (!excludeTestbed) {
		...
		basicFields.crudOptions = crudOptions;
		// Map
		...
		if (doExecute(test, includeFilter, excludeFilter)) {
			...
			for (var t = 0; t < threadCounts.length; t++) {
				...
				for (var j = 0; j < trials; j++) {
					try {
						results[j] = runTest(test, threadCount, multidb, multicoll, seconds, shard, crudOptions, printArgs, cleanUp);
					} catch(err) {
 ...
 * @returns {{}} the results of a run set of tests
 */
function mongoPerfRunTests(threadCounts, multidb, multicoll, seconds, trials, includeFilter, excludeFilter, shard, crudOptions, excludeTestbed, printArgs, cleanUp) {
	testResults = runTests(threadCounts, multidb, multicoll, seconds, trials, includeFilter, excludeFilter, shard, crudOptions, excludeTestbed, printArgs, cleanUp);
	print("@@@RESULTS_START@@@");
